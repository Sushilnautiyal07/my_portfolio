...
export const Navbar = () => {
  ...
  <header className={`fixed top-0 left-0 right-0 transition-all duration-500 ${
      ...
      <div className="hidden md:flex items-center gap-1">
        <div className="glass rounded-full px-2 py-1 flex items-center gap-1">
          {navLinks.map((link,index)=>(
            <a href={link.href} key={index} className="px-4 py-2 text-sm text-muted-forground hover:text-forground rounded-full hover:bg-surface">
              {link.label}
            </a>
          ))}
        </div>
    ...
    {/* mobile menu button */}
    <button className="md:hidden p-2 text-forground " onClick={()=> setIsMobileMenuOpen((prev)=>!prev)}>
